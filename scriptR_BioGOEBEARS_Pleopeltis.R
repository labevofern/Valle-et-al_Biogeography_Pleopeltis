...
#trfn = tree file name
trfn <- "C:/R/arvore_pleopeltis.nwk"

tr = read.tree(trfn)
tr
plot(tr)
title("Pleopeltis phylogeny")
...
#geogfn = arquivo com os dados geográficos
geogfn <- "C:/R/matrix_pleopeltis.csv"
...
runslow = TRUE
resfn = "Pleopeltis_BAYAREALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
...
BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["d","max"] = 4.9999999 #alterei esse parâmetro
#porque estava dando problema, seguindo um comentário do próprio Matzke

BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["e","min"] = 0.0000001
BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["e","max"] = 4.9999999 #alterei esse parâmetro
#porque estava dando problema, seguindo um comentário do próprio Matzke
...
resfn = "Pleopeltis_BAYAREALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "Pleopeltis_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=20, height=30)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Pleopeltis_unconstrained"
...
# States
res2 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.6, splitcex=0.8, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.7, statecex=0.5, splitcex=0.00, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

#######################################################
# Plot ancestral states - BAYAREALIKE+J
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Pleopeltis M0_unconstrained"
...
# States
res1 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.6, splitcex=0.8, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.7, statecex=0.5, splitcex=0.00, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

###############################################
#----------Criando o arquivo pdf para Linux e MACos----------------########
#dev.off()
#cmdstr = paste("open ", pdffn, sep="")
#system.file(cmdstr)
#--------------------------------------
## Criando o arquivo pdf para windows
dev.off()
cmdstr = paste("start ", pdffn, sep="")
system(cmdstr)
#--------------------------------------
...
runslow = TRUE
resfn = "Pleopeltis_DEC_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Pleopeltis_DEC+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
...
# PDF plots
#######################################################
pdffn = "Pleopeltis_DEC_vs_DEC+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=20, height=30)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on Pleopeltis M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DEC+J on Pleopeltis M0_unconstrained"
...
runslow = TRUE
resfn = "Pleopeltis_DIVALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Pleopeltis_DIVALIKE+J_M0_unconstrained_v1.Rdata"
if (runslow)
runslow = TRUE
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
if (runslow) {
  # Executa a análise se runslow for TRUE
  # sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/") 
  ...
  # Carrega os resultados se runslow for FALSE
  load(resfn)
  resDIVALIKEj = res
}
#######################################################
pdffn = "Pleopeltis_DIVALIKE_vs_DIVALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=20, height=30)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on Pleopeltis M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Pleopeltis M0_unconstrained"
...
pdffn = paste0("Pleopeltis_", model_name, "_v1.pdf")
pdf(pdffn, height=20, width=10)

analysis_titletxt = paste0(model_name, " on Pleopeltis")
...
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.7, statecex=0.5, splitcex=0.0, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
...
if (runBSMslow == TRUE)
{
  ...
  BSM_output = runBSM(res, stochastic_mapping_inputs_list=stochastic_mapping_inputs_list, maxnum_maps_to_try=100, nummaps_goal=100, maxtries_per_branch=40000, save_after_every_try=TRUE, savedir=getwd(), seedval=12345, wait_before_save=0.01, master_nodenum_toPrint=0)
...
pdf(file=pdffn, height=20, width=10)
...
pdffn = paste0(model_name, "Pleopeltis_", length(clado_events_tables), "BSMs_v1.pdf")
pdf(file=pdffn, height=20, width=10)
...
for (i in 1:nummaps_goal)
{
  ...
  analysis_titletxt = paste0(model_name, " Pleopeltis- Stochastic Map #", i, "/", nummaps_goal)
